...
The pending source also refreshes the original dark interface: a simpler sidebar, clearer menu and device controls, better fitting Profiles and Settings pages, a timed-license countdown, and separate colored Builder Base Wall and Building totals. The extra Nova theme was removed. These interface changes passed a local build and visual checks at the app's minimum window size; they are not a published customer update yet.
