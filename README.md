...
The pending source also waits for the full Area 2 troop bar before moving the camera to the saved side. Previously it could pan during the area transition, then deploy at saved screen coordinates without a matching camera position. A controlled Right-side live attack showed the Area 2 base in the saved Right-side view, three surviving troops spread outside the base, and a clean return to Builder Base. The same timing path serves all four sides, but Left, Top, and Bottom have not had this change live-tested. The customer download has not been updated yet.
